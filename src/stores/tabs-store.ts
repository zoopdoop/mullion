...
export const tabsReducer = produce(
  (draft: Draft<ITabsState>, action: ITabAction) => {
    const getAppTabIndex = (appTabId: Id) =>
      draft.appTabs.findIndex((appTab) => appTab.id === appTabId);
    const updateAppTab = (
      appTabId: Id,
      updater: (appTab: WritableDraft<IAppTab>) => void
    ) => {
    ...
    switch (action.type) {
      ...
      case "addSecondaryTab":
        ...
        updateAppTab(action.value.appTabId, (appTab) => {
          appTab.secondaryTabs.push(
            createSecondaryTab(secondaryId, action.value.newSecondaryTab)
          );
        ...
        if (numAppTabs > 1) {
          ...
          if (index !== -1) {
            ...
            if (action.value.appTabId === draft.selectedAppTabId) {
              draft.selectedAppTabId = (
                draft.appTabs[index] || draft.appTabs[index - 1]
              ).id;
            }
          }
      ...
      case "navigateToUrl":
        updateAppTab(
          action.value.appTabId,
          (appTab) => (appTab.url = action.value.url)
        );
